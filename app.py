...
def get_vector_store(text_chunks):
    if not text_chunks:
        ...
        return
    try:
        # Use a completely free, local embedding model (no API or quotas)
        embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")

        # Build FAISS vector store as before
        vector_store = FAISS.from_texts(text_chunks, embedding=embeddings)
...
def get_conversational_chain():
    prompt_template = "You are LawPilot... Context:\n {context}\n Question: \n{question}\n Answer:"
...
def generate_summary(custom_instruction):
    ...
    st.session_state.chat_history.append(("LawPilot", f"**Summary (Instruction: *{custom_instruction}*)**\n\n" + final_summary))
...
# --- Main UI ---
st.title("⚖️ LawPilot - Your Legal AI Guide")
...
if prompt:
    ...
    st.session_state.chat_history.append(("LawPilot", response_text))
    st.rerun()